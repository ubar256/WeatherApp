...
extension NetworkRequest {
  ...
  func convertUrlRequest() throws -> URLRequest {
    ...
    let urlQueryToken = URLQueryItem(name: "appid", value: "\(ApiConstants.apiKey.rawValue)")
    let urlQueryUnits = URLQueryItem(name: "units", value: "metric")
    let urlQueryExclude = URLQueryItem(name: "exclude", value: "hourly,daily")
    ...
    guard let components = urlComponents?.url else {throw NetworkError.missingURL}
    var urlRequest = URLRequest(url: components)
    urlRequest.addValue("application/json", forHTTPHeaderField: "Content-Type")
    if !bodyParams.isEmpty {urlRequest.httpBody = try? JSONSerialization.data(withJSONObject: bodyParams, options: [])
